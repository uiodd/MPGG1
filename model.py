...
import math
from mm_gated_attention import MMGatedAttention, MultiModalFusionLayer
from enhanced_gated_attention import EnhancedGatedAttention
...
class Transformer_Based_Model(nn.Module):
    def __init__(self, dataset, temp, D_text, D_visual, D_audio, n_head,
                 n_classes, hidden_dim, n_speakers, dropout, projection_dim=128):
        ...
        self.enhanced_gated_attention = EnhancedGatedAttention(
            mem_dim=hidden_dim,
            cand_dim=hidden_dim,
            num_emotions=n_classes,
            att_type='general',
            dropout=dropout,
            use_cluster_contrastive=True,
            temperature=0.07,
            lambda_cluster=1.0,
            lambda_instance=0.5,
            num_clusters_per_class=2,
            contrastive_dim=projection_dim
        )
        ...
        # Prototype loss
        self.proto_loss = ProtoLoss(feature_dim=projection_dim, num_classes=n_classes)
        self.emotion_prototypes = nn.Parameter(torch.randn(n_classes, projection_dim))

        self.proto_projection_head = nn.Linear(hidden_dim, projection_dim)
    ...
    def forward(self, textf, visuf, acouf, u_mask, qmask, dia_len, emotion_labels=None):
        ...
        if emotion_labels is not None:
            enhanced_gated_out, cluster_contrastive_loss, contrastive_features = self.enhanced_gated_attention(
                a=a_transformer_out,
                v=v_transformer_out,
                l=t_transformer_out,
                modals=['a', 'v', 'l'],
            ...
            enhanced_gated_out = self.enhanced_gated_attention(
                a=a_transformer_out,
                v=v_transformer_out,
                l=t_transformer_out,
                modals=['a', 'v', 'l']
            )
            cluster_contrastive_loss = None
        ...
        enhanced_all_out = (all_transformer_out + enhanced_gated_out) / 2.0
        ...
        early_fusion = t_t_transformer_out + a_a_transformer_out + v_v_transformer_out
        

        final_fusion_feature = enhanced_all_out + early_fusion
        
        all_final_out = self.all_output_layer(final_fusion_feature)
        ...
        proto_loss = None
        if emotion_labels is not None:
            # Project features for prototype loss
            proto_features = self.proto_projection_head(enhanced_all_out)
            b, s, d = proto_features.shape
            proto_features = proto_features.view(b * s, d)
            flat_emotion_labels = emotion_labels.view(-1)
            cluster_prototypes = self.enhanced_gated_attention.cluster_contrastive_loss.emotion_prototypes.mean(dim=1)
            proto_loss = self.proto_loss(proto_features, flat_emotion_labels, cluster_prototypes)

        return t_log_prob, a_log_prob, v_log_prob, all_log_prob, all_prob, \
            kl_t_log_prob, kl_a_log_prob, kl_v_log_prob, kl_all_prob, \
            t_transformer_out, a_transformer_out, v_transformer_out, enhanced_all_out, cluster_contrastive_loss, proto_loss, contrastive_features
